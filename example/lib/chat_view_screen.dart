...
class ChatViewScreen extends StatefulWidget {
  const ChatViewScreen({required this.chat, super.key});
...
class _ChatViewScreenState extends State<ChatViewScreen> {
  ...
  void _showHideTypingIndicator() {
    _chatController?.setTypingIndicator =
        !(_chatController?.showTypingIndicator ?? false);
  }
      ...
      body: _chatController == null
          ...
          : ChatView(
              ...
              appBar: ChatViewAppBar(
                ...
                userStatus: widget.chat.userActiveStatus.isOnline
                    ? 'Online'
                    : 'Offline',
  ...
  void _onSendTap(
    ...
    final messageObj = Message(
      ...
      createdAt: DateTime.now(),
      message: message,
      sentBy: controller.currentUser.id,
      replyMessage: replyMessage,
      messageType: messageType,
    );
    controller.addMessage(messageObj);

    Future.delayed(const Duration(milliseconds: 300), () {
      final index = controller.initialMessageList.indexOf(messageObj);
      controller.initialMessageList[index].setStatus =
          MessageStatus.undelivered;
    });
    Future.delayed(const Duration(seconds: 1), () {
      final index = controller.initialMessageList.indexOf(messageObj);
      controller.initialMessageList[index].setStatus = MessageStatus.read;
    });
  }
